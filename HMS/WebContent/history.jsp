...
st= con.prepareStatement("select b.id, h.name,b.checkindate, bi.`total amount` as totalamount, b.checkoutdate, h.address, r.roomno, b.noofguests from booking b inner join room r on b.roomid = r.id inner join hotels h on h.id = r.hotelid inner join roomtype rt on r.roomtypeid = rt.id inner join bills bi on bi.bookingid = b.id where b.bookingstatus = 1 and b.cusid="+ cus_id +" order by b.id desc");
rs = st.executeQuery();
int i = 1;
while(rs.next()){
%>
